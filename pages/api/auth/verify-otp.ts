...
import { createSessionForRequest, setSessionCookie, setTrustedDeviceCookie } from '@/lib/api-server/auth';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { challengeId, otp, trustDevice = false } = (req.body ?? {}) as { challengeId?: string; otp?: string; trustDevice?: boolean };
  ...
  try {
    ...
    const challenge = result.rows[0] as { id: string; username: string; otp_hash: string; password_required: boolean; remember_me: boolean; attempts: number } | undefined;
    ...
    setSessionCookie(res, await createSessionForRequest(req, challenge.username, await getAdminSessionVersion(challenge.username)), challenge.remember_me);
    if (trustDevice && challenge.password_required) setTrustedDeviceCookie(res, challenge.username);
