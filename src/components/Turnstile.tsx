...
export function useTurnstile(action: string) {
  ...
  const execute = async () => {
    ...
    const isLocalhost = hostname === "localhost" || hostname === "127.0.0.1" || hostname === "0.0.0.0" || hostname === "::1" || hostname.startsWith("127.") || hostname.startsWith("10.") || hostname.startsWith("192.168.") || /^172\.(1[6-9]|2\d|3[0-1])\./.test(hostname);
    if (!siteKey || (process.env.NODE_ENV !== "production" && isLocalhost)) return Promise.resolve("");
    await ensureWidget();
